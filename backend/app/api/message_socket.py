...
from app.models.database import DirectConversation, PrivateMessage, User, UserStatus
...
async def message_socket(websocket: WebSocket) -> None:
    ...
    try:
        ...
        try:
            ...
            return
        await message_gateway.register(user.uid, websocket)
        while True:
            try:
            ...
            conversation = (
                _conversation(db, conversation_id, user.uid)
            ...
            if event_type in {"typing.start", "typing.stop"}:
                if event_type == "typing.start":
                    await message_gateway.typing_start(conversation_id, user.uid, pair)
                else:
                    await message_gateway.typing_stop(conversation_id, user.uid, pair)
                continue
            if event_type == "message.read":
                ...
                unread_messages = db.exec(
                    select(PrivateMessage).where(
                        ...
                        PrivateMessage.receiver_uid == user.uid,
                ...
                db.commit()
                await message_gateway.broadcast(conversation_id, {"type": "message.read", "conversation_id": conversation_id, "message_id": message_id, "reader_uid": user.uid}, recipient_uids=pair)
                continue
            await _error(websocket, "INVALID_EVENT", "不支持的事件类型")
    finally:
        try:
            if user is not None:
                await message_gateway.unregister(user.uid, websocket)
        finally:
            db.close()
