...
async function load() {
  ...
  error.value = ''
  try {
    const result = await http.get<PageResult<Post>>('/posts', {
      params: {
        page: page.value,
        page_size: 10,
        sort_by: 'created_at',
              ...
              <div class="flex items-start justify-between gap-4">
                <div class="min-w-0 flex-1">
                  ...
                  <div class="mt-3 flex items-center gap-2 text-xs text-slate-400">
                    ...
                    <span>{{ notice.author?.username ?? '管理员' }}</span>
                    <span>·</span>
                    <span>{{ formatChinaDateTime(notice.created_at) }}</span>
                  </div>
                </div>
                <span
                  v-if="isAdmin && (notice.author_uid === authStore.user?.uid || authStore.user?.role === 'admin' || authStore.user?.role === 'root')"
                  class="flex-shrink-0"
