...
const notice = ref<Announcement | null>(null)
...
async function load() {
  ...
  error.value = ''
  try {
    notice.value = (await announcementsApi.get(Number(route.params.id))).data
  } catch (reason) {
...
async function deleteNotice() {
  ...
  try {
    await announcementsApi.delete(notice.value.id)
      ...
      <template v-else-if="notice">
        <article class="glass-card p-7 sm:p-9">
          ...
          <div class="flex flex-wrap items-center gap-2 text-sm text-slate-500">
            ...
            <span>·</span>
            <time>{{ formatChinaDateTime(notice.published_at || notice.created_at) }}</time>
          </div>
          ...
          <div class="mt-8 flex items-center justify-between gap-4 border-t border-slate-200 pt-4 dark:border-neutral-800">
            <span class="text-sm text-slate-400">优先级: {{ notice.priority }}</span>
